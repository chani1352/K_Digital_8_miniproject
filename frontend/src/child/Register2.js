...
export default function Register2({ info }) {
    ...
    const makeTable = () => {
        const tags = vaccineList.map(item => 
            <tr className="bg-white border-b" key={item.idx[0]}>
                <th scope="row" className="px-6 py-4 font-medium text-gray-900 ">{item.optional}</th>
                <td className="px-6 py-4">{item.disease}</td>
                <td className="px-6 py-4">{item.code}</td>
                <td className="px-6 py-4">{item.idx.map(idx => 
                        <input id="default-checkbox" type="checkbox" value={idx}
                        key={idx}
    ...
    const putVaccineList = async(idx) => {
        ...
        //localhost:8080/selectVaccine?child_idx=21&vaccine_idx=1,2,3
